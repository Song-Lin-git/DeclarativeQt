...
LambdaList: Grammar = lambda lt, exp: list([exp(x) for x in lt])
ConditionList: Grammar = lambda lt, condition: list([x for x in lt if condition(x)])
EnumList: Grammar = RGrmBase.GEnumList
DtLambdaList: Grammar = lambda dt, exp: list([exp(k, v) for k, v in dt.items()])
...
LambdaDict: Grammar = lambda iterable, keyExp, valExp: dict({keyExp(x): valExp(x) for x in iterable})
...
DtLambdaDict: Grammar = RGrmBase.BaseMethods.DtLambdaDict
...
MaxOfList: Grammar = lambda lt, exp: max(LambdaList(lt, exp))
MinOfList: Grammar = lambda lt, exp: min(LambdaList(lt, exp))
